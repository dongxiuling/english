...
class Voice_model extends CI_Model {
    ...
    public function do_select($words_id){
       $pdo = DB::getInstance();
        $sql = "select aa.*,collectvoice.flag from (select voice.voice_id,t_user.user_name,voice.url,t_user.user_id,words.words_id,voice.date from voice,t_user,words where voice.user_id = t_user.user_id and voice.words_id = '$words_id' order by date desc) aa left join collectvoice on aa.voice_id =collectvoice.voice_id";
